...
## Leverage-flush mean reversion (long-only) — a strategy under test
...
## Adaptive DCA (volatility harvesting) — an ACCUMULATION simulator, not a trade backtester

A third, independent module (`longbot/accumulation.py`, `longbot/dca_metrics.py`,
`run_dca_simulation.py`). This one is fundamentally different from the two
strategies above: **there are no trades.** No entries, no exits, no stops,
no expectancy — and **nothing is ever sold**. It only ever buys. Because of
that, it is judged by a completely different, honest standard: not "did it
beat the market" but "did buying more on fear achieve a better cost basis
and money-weighted return than just buying the same fixed amount every day
— and is that edge big enough to bother with?"

### The idea (behavioral, not an alpha edge)

Retail investors systematically buy during euphoria and sell during panic —
which is a large part of why the average investor underperforms the very
assets they hold. This mechanism does the opposite, mechanically: it buys
**more** on red days, buys **less or nothing** on euphoric green days, and
never sells. It is not trying to time tops or bottoms. It is harvesting a
documented *behavioral* tendency — one that persists precisely because it's
a behavior, not a priced market inefficiency that would get arbitraged away.
Long-only, spot, never-sell → trivially halal (no shorting, no riba, no
leverage, not even a sell decision to make).

### The pre-registered ladder (fixed — not tuned to flatter results)

Once per day, using the return of the last **completed** bar, buy this USD
amount, filled at the **next** bar's open (see `longbot/accumulation.py`):

| Daily return `r` | Buy amount |
|---|---|
| `r > +5%` | $0 (skip — euphoria) |
| `−2% ≤ r ≤ +5%` | $5 (base) |
| `−5% < r < −2%` | $7 |
...
A 0.10% spot buy fee applies to every purchase (it genuinely raises the
realized cost basis slightly above the raw price paid — see the tests). No
slippage modelled at this size/frequency.

### Three approaches, compared honestly

1. **Adaptive DCA** — the ladder above.
2. **Flat DCA** — a constant $5 every day, on the *same* execution days as
   adaptive. This is the fair fight: same behavioral approach, dumb sizing
   vs. adaptive sizing.
3. **Lump-sum** — adaptive's total deployed capital, all at once on day 1.
   Context only, **not** the fair fight — lump-sum tends to win in a rising
   market simply because markets rise more often than they fall, which has
   nothing to do with whether the ladder logic itself is any good.
4. **Flat DCA, same total capital as adaptive** — adaptive's total spend,
   spread evenly across *only the days adaptive actually bought on*. This
   isolates the effect of variable **sizing** from the fact that adaptive
   simply deploys a different total amount of capital than flat $5/day does.

### No-lookahead

The buy amount for execution day `t+1` is decided from the return realized
on the **completed** bar `t` (`close[t]/close[t-1] − 1`) — bar `t+1`'s own
high/low/close are never read to decide bar `t+1`'s own purchase. Proven by
truncation- and future-replacement-invariance tests in
`tests/test_accumulation.py`, the same style used for the other two
strategies in this repo.

### Metrics (accumulation-appropriate — deliberately NOT trade expectancy)

Nothing is sold, so there is no realized P&L per trade to report. Instead:
total capital deployed, total units accumulated, **average cost basis**
(the cleanest, most capital-neutral test of the thesis), **money-weighted
return (XIRR)** solved by bisection (no scipy/numpy-financial dependency —
see `longbot/dca_metrics.xirr`), mark-to-market **portfolio value** and its
peak-to-trough **drawdown** (read as "how far underwater did the current
stack get", not a pure return drawdown, since new capital keeps arriving),
and capital-deployed + cost-basis broken down **per year** and **per BTC
macro regime** (BULL/BEAR/CHOP, reusing `btc_regime_labels`).

### Assets — including an honest risk demonstration

- **BTCUSDT** and **ETHUSDT** — the never-sell "this asset will survive and
  eventually rise" assumption is at least defensible for the majors.
- **ETCUSDT**, deliberately, as a **cautionary** alt: it peaked near $176 in
  2021 and has spent years since at a small fraction of that high, with no
  new ATH. It is included specifically to demonstrate this strategy's core
  **unmanaged risk** — "buy more on the way down" is a genuine disaster if
  the asset never recovers, because this strategy has no mechanism to ever
  stop buying, or to sell. Its result is reported as a warning, not a bug.

### Framing the verdict (NOT a trading pass/fail)

Do **not** judge this by "did it beat buy-and-hold / lump-sum on total
return" — in a bull market, a dip-buyer that holds cash waiting for dips
will often trail a strategy that was never holding cash, and that is the
strategy working exactly as designed, not failing. `run_dca_simulation.py`
answers four questions instead, computed fresh from whatever data it's fed
(never hard-coded):

1. Did Adaptive DCA achieve a **lower average cost basis** than Flat DCA? By how much?
2. Did Adaptive DCA achieve a **better money-weighted return (XIRR)**, net of fees?
3. Was the advantage **concentrated in the volatile/BEAR regime**, as the thesis predicts?
4. Is the advantage **large enough to justify the added complexity** — or is flat DCA enough?

If adaptive only marginally beats flat DCA, "just do flat DCA — simpler and
nearly as good" is a valid, useful, non-failure conclusion.

### Reproduce

```bash
pip install -r requirements.txt
python -m pytest tests/ -v          # no network needed — synthetic + hand-built OHLCV
python run_dca_simulation.py        # needs api.binance.com; runs BTCUSDT, ETHUSDT, ETCUSDT from 2017/2020->now
```

Built and tested in a sandboxed environment with no outbound network access
— it has **not** been run against real data. The four framing questions
above are computed mechanically from whatever numbers come back; paste them
back for a plain-language read, including whether the ETCUSDT cautionary
case shows the never-sell risk playing out as expected.
